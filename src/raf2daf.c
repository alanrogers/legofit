...
# Usage
    ...
    Usage: raf2daf [options] <x>=<in1> <y>=<in2> ...
    ...
    Options may include:
       -f <name> or --bootfile <name>
          Bootstrap output file basename. Def: raf2daf.boot.
       -r <x> or --bootreps <x>
...
Before running `raf2daf`, use @ref raf "raf" to convert the input data
...
const char *useMsg =
    "\nUsage: raf2daf [options] <x>=<in_1> <y>=<in_2> ... outgroup=<in_K>\n"
...
int main(int argc, char **argv) {
    ...
    if(bootfname[0] == '\0') {
        const char *defName = "raf2daf.boot";
    ...
    if(bootreps > 0) {
        ...
        for(j = 0; j < bootreps; ++j) {
            ...
            fprintf(fp, "# %13s %20s", "Raf2daf", "E[count]\n");
            for(i = 0; i < npat; ++i) {
    ...
    printf("# %13s %20s", "Raf2daf", "E[count]");
    if(bootreps > 0)
