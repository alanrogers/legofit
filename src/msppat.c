...
# Msppat: tabulates site patterns

Msppat reads data generated by msp (with option -transpose-segsites)
...
# Usage
    ...
    Usage: msppat [options] <x> <y> ...
      where <x>, <y>, etc. are arbitrary labels, whose number and order
      must agree with that of the populations specified in the msp command
      line (using msp arguments -I and -eI). Labels may not include the
...
`msppat` parses a file generated using `msp`. The `msp` command
...
by `msp`, and that these simulated data included genotypes referring
to four populations, labeled "x", "y", "n", and "d". The `msppat`
command would look like this:

    msppat --infile foo.msp x y n d

`msppat`'s notion of a "population" differs from that of `msp`, in
that `msppat` treats samples of different ages as separate
populations, even if they reside in the same population on the `msp`
command line. For example, consider the following `msp` command line:

    msp 3 -I 2 1 1 -eI 0.5 0 1
...
To generate a bootstrap, use the `--bootreps` option:
    ...
    msppat --bootreps 50 --infile foo.msp x y n d > obs.txt
...
int main(int argc, char **argv) {
    ...
    if(ifp!=stdin)
        fclose(ifp);
    fprintf(stderr, "mscpat is finished\n");
    return 0;
}
