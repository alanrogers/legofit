...
@note 
Ryan Bohlender 10-6-2017
Assuming a file sorted by position, within each chromosome, we can compare duplicates
as they will always be adjacent. Given a qual value strictly greater than the previous
line, we will print the current line, otherwise the previous line. To handle multiple
duplicates, we will output the line upon reading a new position, or EOF.
...
int main(int argc, char **argv) {
    ...
    while(1) {
        ...
        if(lastnucpos) {
            if(lastnucpos == nucpos) {
                ...
                */
                ...
                // Print previous line and continue
                printf(lastline);
            }
        }
        lastnucpos = nucpos;
        ...
        if(aai == 1)
            x = n - x;
        double      p = x / ((double) n);
        snprintf(lastline, 100, "%4s %10s %2s %2c %20.18f\n",
                 chr, pos, aa, alleles[1 - aai], p);
    }
    // We will skip the final line with the new logic, so print it here.
    printf(lastline);
