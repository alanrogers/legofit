...
def sub_input(input,string):
    return [i for i in input if i.split()[0] == string]
...
class lego_tree:
    def __init__(self,name,input):
        ...
        for i in self.full_seg:
            seg_time[i.split()[1]] = i.split()[2].split('=')[-1]
        for i in self.pieces:
...
if __name__ == "__main__":
    ...
    try:
        ...
        allmix = True
    try:
        text_size = float(arg_vals["text_size"])
    except:
        text_size = 12
    try:
    ...
    for i,tip in enumerate(tree.tips):
        ...
        plt.plot(line_x,line_y,solid_capstyle="round",linewidth = 30,color = 'white',label = dest.name,zorder =2)
