...
int main(int argc, char **argv) {
    ...
    // Make BranchTab to store global site pattern frequencies. This
    // is needed only for debugging. I want to make sure that the
    // site pattern frequencies calculated here match those in the file
    // generated by sitepat.
    unsigned nsamples = LblNdx_size(lgosim_lndx);
    ...
    if( BranchTab_normalize(bt) )
      fprintf(stderr,"%s:%d: warning: can't normalize empty BranchTab\n",
              __FILE__,__LINE__);

    // Put site patterns and branch lengths into arrays.
    if(npat != BranchTab_size(bt)) {
      fprintf(stderr,"%s:%d: mismatch in number of patterns.\n",
              __FILE__,__LINE__);
    }else{
      long double frq[npat];
      BranchTab_toArrays(bt, npat, pat, frq);
