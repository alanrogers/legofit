...
int main(int argc, char **argv) {
    ...
    unsigned npat = (1UL << m) - 2UL;  // number of site patterns
    if(!doSing)
        npat -= m;
    printf("# %s singleton site patterns.\n",
           (doSing ? "Including" : "Excluding"));
    printf("# Number of site patterns: %u\n", npat);
    tipId_t     pat[npat];
    ...
    // prpat[i] is the probability of pattern i at the current
    // nucleotide site.
    //
    // patfrq[i] is the frequency of pattern i across all sites.
    double prpat[npat];
    long double patfrq[npat];
    memset(prpat, 0, npat * sizeof(prpat[0]));
    memset(patfrq, 0, npat * sizeof(patfrq[0]));
    ...
    while( !done ) {
        ...
        for(i = 0; i < npat; ++i) {
            ...
            // pattern i at the current nucleotide site. It would sum
            // to 1 if we included all site patterns, including the
            // two monomorphic ones. But since we don't include those,
            // and we may not be including singletons either. The
            // distribution must be normalized. So this loop
            // calculates the unnormalized probabilities along with
            // their sum. The loop that follows will normalize.
            sumz += z;
            prpat[i] = z;
        }
        for(i = 0; i < npat; ++i) {
            prpat[i] /= sumz;  // normalize

            // cond_pr[i] is conditional probability of admixture
            // given site pattern i. pr is the probability of
            // admixture in a random haploid sample drawn from the
            // current nucleotide site.
            pr += prpat[i] * cond_pr[i];
            patfrq[i] += prpat[i];
        }
    ...
    // calculate mean admixture again to see whether it matches
    ...
    for(j = 0; j < npat; ++j) {
        ...
        if(pat[ord[j]] == union_all_samples)
            continue;
        snprintf(buff2, sizeof(buff2), "%s",
                 patLbl(sizeof(buff), buff, pat[ord[j]], &lndx));
        printf("%15s %10.7Lf\n", buff2, patfrq[ord[j]]);
    }
